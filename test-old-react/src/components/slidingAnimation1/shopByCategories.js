...
export default class ShopByCategoriesV2 extends Component {
  ...
  render() {
    ...
    return (
      <div className="shopByCat-main-2">
        <div className="mobile-shopByCat-container-2">
          <div
            onClick={this.changeItemsLeft}
            className={this.state.showIndex > 0 ? 'active-mover-btn-icon-2' : 'deactive-mover-btn-icon-2'}
          >
            left
          </div>
          <div
            className="mobile-shopByCat-grid-wrapper-2"
            onTouchStart={this.handleTouchStart}
            onTouchEnd={this.handleTouchEnd}
          >
            <div ref={this.mobileShopByCatGrid} className="mobile-shopByCat-grid-2">
              {itemsToShow.map((e, index) => {
                const { side } = this.shouldRenderDivideLine(index, window.innerWidth, itemsToShow);
                return (
                  <div key={index} className="shopByCat-item-2">
                    <img className="mobile-shopByCat-img-2" src={e.imageMobile} alt="shopByCat Detail" />
                    <div className="shopByCat-title">{e.subDepartmentName}</div>
                    {side && <div className="sbc-side-divideLine-2" />}
                  </div>
                );
              })}
            </div>
          ...
          <div
            ...
            className={this.state.showIndex + 8 < this.state.shopByCat.length ? 'active-mover-btn-icon-2' : 'deactive-mover-btn-icon-2'}
          >
            right
          </div>
        </div>
